...
| `LinkCard` | Rich external-link card used inside message bubbles — icon tile + title/subtitle. | `link: { source, title, subtitle, url }` | `source: 'jira' \| 'github' \| 'figma'`. Figma renders the multi-color Figma logo on a dark tile. Extend the icon set inside the component when a new source is needed. Note: Figma links now render as `FigmaWidget` by default (see below). |
| `FigmaWidget` | Embedded Figma design preview inside message bubbles. Shows live Figma file/frame in an iframe with header (logo + title/subtitle + open link). | `link: { source, title, subtitle, url, embedUrl }` | Automatically used for all `link.source === 'figma'` in messages. The `embedUrl` should be the Figma embed URL (e.g., `https://www.figma.com/embed?embed_host=teams&url=<encoded-file-url>`). Preview uses 16:9 aspect ratio. Header matches Teams card style with Figma's multi-color logo. |
